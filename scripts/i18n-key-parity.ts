...
//
// Estado nesta abertura de branch: só existe pt-BR → 0 locales verificados,
// guarda VERDE e ARMADO. A partir do 1º commit de `messages/es.json`, qualquer
// chave a menos/a mais quebra o gate. Roda no `npm run gate` (lógica pura, sem
// build). Regra de ouro B7 preservada: este script não lê números da matriz.
// ============================================================
...
for (const f of files) {
  ...
  if (missing.length) failures.push(`[${locale}] ${missing.length} chave(s) AUSENTE(s): ${missing.slice(0, 12).join(', ')}${missing.length > 12 ? ' …' : ''}`);
  if (orphan.length) failures.push(`[${locale}] ${orphan.length} chave(s) ÓRFÃ(s): ${orphan.slice(0, 12).join(', ')}${orphan.length > 12 ? ' …' : ''}`);
}

console.log('=== i18n key-parity — messages/<locale> × pt-BR (B2) ===');
console.log(`  referência: ${REFERENCE}.json (${refKeys.size} chaves) · locales verificados: ${checked}`);

if (failures.length) {
  console.log(`\nFALHOU: ${failures.length} divergência(s) de chave:`);
  for (const x of failures) console.log('  ✗ ' + x);
  process.exit(1);
}
console.log(`\nOK: paridade de chaves íntegra (sem ausente/órfã) em ${checked} locale(s).`);
