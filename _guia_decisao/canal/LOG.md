...
### #75 · 2026-09-25 · Operador (coordenador do GT) → canal (registrado pela Engenharia; aplicação na branch `feat/carimbo-beta1`) · [decisão]

**Carimbo de versão exibido a pessoas: «Beta 1 (2.2.0)»** (pendência deixada no #74). Grafia escolhida pelo coordenador entre as opções «Beta_uno 2.2.0» e «Beta 1 (2.2.0)»; alcance: só o que pessoas leem. **Aplicação:** constante `MATRIX_VERSION_LABEL`, derivada de `MATRIX_VERSION` (nunca divergem), exibida na tela de resultado («Versão da matriz: Beta 1 (2.2.0)»), no relatório HTML/TXT (pt e es) e no registro CSV/TXT. **Inalterados de propósito:** `matrixVersion` 2.2.0 da spec e o campo `versaoMatriz` = "2.2.0" nos JSON (validação v3 e registro-espelho v2) — dado de máquina, lido pela planilha e comparado entre CEPs; sem mudança de schema. Cálculo, cortes e ids intocados. Cadeia: build · verify 105/105 + 83/83 · parity 128/0 · gate OK · strict 470/470 · parity:locale 4/4. Texto es: nenhum novo («Beta 1» igual nos dois idiomas; rótulo «Versión de la matriz:» já aprovado).

---

### #74 · 2026-09-25 · Operador (coordenador do GT) → canal (registrado pela Engenharia; aplicação na branch `fix/versao-beta-nota`) · [decisão]

**Duas correções de conteúdo pt por ordem expressa do coordenador.** (1) **Aviso da tela inicial** (`home.disclaimerVersao`): «Esta é a versão 2.0 da matriz.» → «Esta é a versão **Beta** da matriz.» (es: «Esta es la versión Beta de la matriz.»), para não sugerir numeração concorrente com a versão 1.0 do Guia aprovada pelo colegiado da INAEP em 16/09/2026. O restante da frase segue igual. **Não alterados:** `matrixVersion` 2.2.0 e o carimbo «Versão da matriz: 2.2.0» da tela de resultado, do relatório e dos exports (identificador técnico de auditoria; mudança exige ordem expressa própria). (2) **Nota técnica, seção 2.1** (achado a do #72): a frase de diligência, inserida no meio do período, volta ao fim — só reordenação, mesmas palavras (conferido por script); es espelhado no mapa, sem texto novo. **Registro de processo:** o PR #45 foi mergeado em `f03e262`, antes dos 4 commits do parecer #73 (ajuste «los frentes», glossário v0.6.0, **ativação dos 6 baixáveis es**, LOG #73) — em produção os links es ainda servem o pt com «(en portugués)». Esta branch foi montada sobre `origin/feat/es-baixaveis` (`d8d0f8f`) e leva também esses 4 commits. Cadeia sobre o conjunto: build · verify 105/105 + 80/80 · parity 128/0 · gate OK · strict 470/470 · parity:locale 4/4 (baseline recapturada; diff só na linha do aviso). **Para o Z (ciência):** «versión Beta» é o único texto es novo (cognato); a nota es só foi reordenada. **Seguem com a Direção:** achados b–e do #72 e se o carimbo «Versão da matriz: 2.2.0» deve ganhar rótulo «Beta».

---

### #73 · 2026-09-25 · Z → canal (transcrito pelo Operador; artefato `PARECER_Z_baixaveis-es_2026-09-25.md` versionado neste commit) · [parecer]

**PARECER ES-DL: tradução es dos 6 baixáveis e da orientação do JSON APROVADA, com 1 AJUSTE es-only e ativação autorizada.** Recibo próprio do Z: não-ativação conferida; âncora de cortesia verbatim nos 5 docx, na planilha e no JSON; JSON com valores de schema pt e mapeamento de entrada documentado. Decisões: (1) JSON canônico pt (dado, não UI) — mantido; (2) **"frente" no masculino** — ajuste nas 3 chaves das messages («las tres frentes» → «los tres frentes»); (3) 8 termos novos aprovados, «Diligencia (devolución)» com blindagem contra o sentido de preenchimento; (4) âncora encerrada; (5) referências no original; (6) a tradução espelha o defeito do pt da nota técnica §2.1 até a Direção corrigir o canônico; (7) P4.1/P4.2 com o léxico da spec; (8) moldura das instruções aprovada. **Aplicação pela Engenharia:** `13d0811` (ajuste; em `s2intro` também a concordância decorrente «una»→«uno», «Todas»→«Todos»), `9453252` (glossário **v0.6.0**, 81/81 `aprovado-z`), `6b9cf29` (**ativação**: `TRADUZIDOS.es` com os 6 ids). Cadeia: build · verify 105/105 + 80/80 · parity 128/0 · gate OK · strict 470/470 · parity:locale 4/4. **Smoke es** (build com `LOCALES_ENABLED=true`): as 3 páginas es servem os 6 `-es` (HTTP 200), sem «(en portugués)» nem `hrefLang`; páginas pt nos canônicos. **Registro de processo:** o PR #44 foi mergeado em `5320eca`, antes do parecer de nomenclatura (#71); os 4 commits seguintes da `chore/nomenclatura-guia` (ajuste «justificación», glossário v0.5.0, LOG #71, cabeçalho do roteiro) seguem em PR próprio ou dentro deste. Aceite da arquitetura recomendado pelo Z; merge por decisão da Direção.

---

### #72 · 2026-09-25 · Engenharia (Claude) → canal · [aviso]

**Pendência ES-DL (#71) implementada na branch `feat/es-baixaveis` (empilhada sobre `chore/nomenclatura-guia`); traduções aguardam parecer do Z antes de serem ligadas.** Escopo definido pelo Operador: os 6 baixáveis + JSON, com aviso "(en portugués)" enquanto não houver tradução aprovada. **Entregue:** (1) `src/lib/downloads.ts` — registro único dos baixáveis; em es o link segue para o arquivo pt com o marcador `idioma.arquivoEmPt` e `hrefLang="pt-BR"` até o id entrar em `TRADUZIDOS.es` (hoje vazio); pt-BR byte-idêntico (parity:locale 4/4); (2) instruções A/B es geradas da spec (os 344 textos já aprovados) — pt regenerado byte-idêntico no XML; (3) planilha-modelo es (abas, fórmulas, valores «Sí/No/NO EVALUABLE», cláusula de cortesia) — equivalência pt×es em 561 células calculadas com os mesmos dados, 0 divergência; (4) JSON de validação v3: só `observacao`/`comoUsar` seguem o idioma — campos, valores e schema idênticos (verify-coverage §15); (5) roteiro, nota técnica e suplemento es por mapas pt→es versionados em `scripts/traducao-docx/mapas/` (242 segmentos, formatação preservada, referências bibliográficas no original); (6) teste que garante o gêmeo `-es` para os 6 (ativação sem 404, §16). Revisão independente prévia (agente): 2 HIGH + 4 menores, todos corrigidos; 0 erro de número/limiar/id. Cadeia: build · verify 105/105 + 80/80 · parity 128/0 · gate OK · strict 470/470 · parity:locale 4/4. **Também:** `fc4838c` (branch `chore/nomenclatura-guia`) corrige o cabeçalho de página do roteiro pt, que ainda dizia «Guia… da IA… (Caderno 2)» — resíduo do #69. **Achados no pt canônico, para a Direção (não alterados; traduzidos fielmente):** (a) nota técnica, seção 2.1, item «Questões eliminatórias»: frase de nota de rodapé inserida no meio do período («…pela MARIAH” Somam-se ainda itens de diligência… mérito., saída categorial…»); (b) nota técnica, seção 2: «A matriz, agora na versão do Guia para o GT (Junho/2026)… não reflete a deliberação colegiada» — desatualizado após a aprovação pela INAEP; (c) suplemento, fecho: «antes do envio do Guia à Editora MS» e «período de leitura» — idem; (d) roteiro, Referências: «SAMUELS, R.; OLIVEIRA, A. S. (Org.). Diretrizes éticas para pesquisa com inteligência artificial… no prelo» — título antigo do Guia?; (e) linhas «Versão preliminar · Junho/Julho de 2026» das notas e âncora/badge «Versão preliminar» (já no #70). **Para o Z:** novas frases es (moldura das instruções, planilha, JSON, 3 anexos) + inconsistência de gênero «las frentes» (messages, já aprovado) × «los frentes» (anexos). **Ativação** (preencher `TRADUZIDOS.es`) só após o parecer.

---

### #71 · 2026-09-25 · Z → canal (transcrito pelo Operador; artefato `PARECER_Z_nomenclatura_2026-09-25.md` versionado neste commit) · [parecer]

**PARECER DE NOMENCLATURA das 16 chaves (#70): 15 APROVADAS, 1 AJUSTE** (`pages.validacao.roteiroDesc`: «justificativa» → «justificación», lusismo). Decisões: (1) título do Guia em es **mantém «de la Inteligencia Artificial»**; glossário atualiza só o `termoPtBr` para o nome oficial; (2) **«Caderno Referencial» não se traduz** até ato da Direção (entrada nova, «el Caderno Referencial» quando articulado); (3) «revisión editorial» aprovado, sem entrada de glossário (status institucional); (4) entradas novas «documento MARIAH» e «Seção de Validação Local → Sección de Validación Local»; (5) «Instrucciones de cumplimentación» aprovado; (6) registro INAEP e s5p confirmados. Ciência: âncora do disclaimer e badge «Versão preliminar» intocados — atualização futura é pedido próprio (glossário + âncora no mesmo ato). **Aplicação pela Engenharia:** `405b393` (ajuste) e `5541ab6` (glossário **v0.5.0**, 72/72 `aprovado-z`); cadeia verde (verify 105/105 + 70/70 · parity 128/0 · gate OK · strict 469/469). **Cumprido o encaminhamento, o parecer deixa de obstar o PR — merge por decisão da Direção.** **Achados da Engenharia para próximo ciclo (não alterados):** (a) `pages.transparencia.notaTecnicaDesc` (es, já aprovado) traz «justificativas» — mesmo lusismo; submeter ao Z; (b) `glossario-es.md` (vista humana) parado na v0.2.0; (c) **pendência nova ES-DL, levantada pelo Operador:** na versão es, todos os baixáveis estão só em pt — planilha-modelo (.xlsx), roteiro de Validação Local, instruções A/B, nota técnica e suplemento (.docx) — e o JSON de validação v3 tem `observacao`/`comoUsar` fixos em pt (relatório HTML/TXT e registro-espelho já seguem o idioma). Escopo e prioridade a definir pela Direção.

---

### #70 · 2026-09-25 · Direção → canal (decisões registradas pela Engenharia; aplicação em `2f8269e` + `fc401db`, branch `chore/nomenclatura-guia`) · [decisão]

**Encerradas as duas deliberações que o #69 deixou abertas, mais a nomenclatura e o status do conjunto aprovado.** Decisões da Direção: (1) **Nomes oficiais:** *Guia de Uso Ético de Inteligência Artificial em Pesquisa com Seres Humanos* · *Caderno Referencial* · *MARIAH*. O conteúdo antes citado como «Caderno 2» / «Apêndice de Validação Local» pertence ao **documento MARIAH** («Seção de Validação Local do documento MARIAH»). (2) **Status:** aprovados pela INAEP em 16/09/2026; em revisão editorial para publicação (substitui «em revisão pelo GT» / «versão preliminar» nos avisos de status das páginas e anexos). (3) **Versão A não avaliável na planilha: aceitar e medir à parte** — planilha-modelo **v2.1**: a aba «Versão A» aceita «NÃO AVALIÁVEL»; o kappa usa só os pares com nível I–IV; nova medida «concordância quanto à avaliabilidade» (Painel). Verificação da Engenharia: 561 fórmulas recalculadas, 0 erros; 8 pares de teste conferidos à mão (kappa 0,3077; avaliabilidade 0,875). `comoUsar.versaoA` do JSON v3 alinhado (sem mudança de schema). **Aplicação:** 16 chaves pt + 16 es; `observacao` do JSON; roteiro e nota técnica («Caderno 2» → «documento MARIAH»); instruções A/B regeneradas (só a linha de status); baseline NDTI recapturada (diff restrito aos textos acima). Cálculo do nível, cortes, ids, `matrixVersion` e âncoras do disclaimer **intocados**. Cadeia: build · verify 105/105 + 70/70 · parity 128/0 · gate OK · strict 469/469 · parity:locale 4/4. **Condição de merge:** os 16 textos es são rascunho — **o PR só entra no main depois do parecer do Z** (o es está publicado). **Continuam para a Direção:** (a) âncora do disclaimer «Versão preliminar: … aguardando validação institucional pelo Ministério da Saúde» e badge «Versão preliminar» — texto protegido, não alterado; exige ordem expressa + parecer do Z (âncora verbatim ao glossário); (b) linhas de edição das notas técnicas («Versão preliminar · Junho/Julho de 2026») mantidas como registro histórico; (c) QUADRO sem atualização desde o #65.
